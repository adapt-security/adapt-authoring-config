...
class ConfigModule extends AbstractModule {
  ...
  */
  async initialise() {
    try {
      ...
      console.log(e);
      process.exit(1); // special case, but exit here
    }
  }
  /**
  ...
  */
  ...
  envVarToConfigKey(envVar) {
    if(envVar.startsWith('ADAPT_AUTHORING_')) {
      return envVar.replace('__', '.').replace(/_/g, '-').toLowerCase();
    }
    return `env.${envVar}`;
  }
  /**
  ...
  */
  async storeSchemaSettings() {
    ...
    if(errors.length) {
      throw new Error(`config settings for the '${process.env.NODE_ENV}' environment failed validation. See below for details:\n${errors.join('')}`);
    }
  }
  /**
